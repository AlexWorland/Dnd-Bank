...
```bash
mvn spring-boot:run           # run the API + UI locally
docker compose up --build     # launch app, Postgres, and backup worker
mvn test                      # execute unit/integration tests when added
```
...
Follow the conventional commits pattern (`feat(accounts): add Zelle transfer`). Each PR should explain player vs. DM impact, list test evidence (`mvn test`, docker smoke checks), and include UI screenshots when templates change. Link campaign/backlog issues and wait for CI to pass before merging.
